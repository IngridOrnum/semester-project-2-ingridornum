import{c as k,A as $}from"./input-CCXIZeBy.js";import{g as S,r as L}from"./main-B8-7TrVe.js";import{a as T}from"./read-BRyaL7qt.js";import{g as A,h as H,t as j,f as C}from"./listings-B-nIuWtN.js";async function N(s,e){const r=await S();if(!r)throw console.error("No access token found."),new Error("Unauthorized: No access token.");const l={method:"POST",headers:new Headers({"X-Noroff-API-Key":k,Authorization:`Bearer ${r}`,"Content-Type":"application/json"}),body:JSON.stringify({amount:e})},a=await fetch(`${$}/${s}/bids`,l);if(!a.ok){const c=await a.text();throw console.error("Failed to place bid:",a.status,a.statusText,c),new Error(`Error placing bid: ${a.status}`)}return await a.json()}document.querySelectorAll(".dropdown-btn").forEach(s=>{s.addEventListener("click",()=>{const e=s.nextElementSibling;e.style.display=e.style.display==="flex"?"none":"flex"})});async function P(){const s=localStorage.getItem("loggedInUsername");if(!s)throw new Error("No logged-in user found");return(await L(s)).data.credits}async function U(){const s=localStorage.getItem("listingId");if(!s){console.error("no listing id found");return}try{let I=function(i){const o=document.getElementById("carousel");let n="";i.length>0?(n=i.map((t,u)=>`
                    <img class="carousel-image" src="${t.url}" alt="${t.alt||"Image"}" style="display: ${u===0?"block":"none"}">
                `).join(""),o.innerHTML=n,i.length>1&&(B(),E())):o.innerHTML='<img class="carousel-image" src="../../../../public/assets/images/missing-img.jpg" alt="No image available" style="display: block;">'},B=function(){document.getElementById("carousel").insertAdjacentHTML("beforeend",`
    ...
    `;const h=document.getElementById("listing-description");h.innerHTML+=`
        ...
        `;const p=document.getElementById("listing-bid-history");async function v(){if(e.data.bids.length>0){const i=e.data.bids.sort((n,t)=>new Date(n.created)-new Date(t.created)),o=await Promise.all(i.map(async(n,t)=>{const u=await C(n.created),f=t===i.length-1?"font-medium":"";return`
            ...
            `}));p.innerHTML=o.join("")}else p.innerHTML="<li>No bids have been placed yet.</li>"}v();const w=document.getElementById("listing-about-seller");w.innerHTML+=`
        ...
        `,document.querySelector(".visit-profile").addEventListener("click",()=>{localStorage.setItem("profileUsername",e.data.seller.name),window.location.href="../../../../profile/"}),I(e.data.media),!d&&!a&&!c&&document.getElementById("place-bid-button").addEventListener("click",async()=>{const o=document.getElementById("bid-amount"),n=parseFloat(o.value);if(isNaN(n)||n<=0){alert("Please enter a valid bid amount.");return}if(n<=r){alert(`Your bid must be higher than the current highest bid of ${r} credits.`);return}if(n>g){alert("You have not enough credits to place this bid.");return}try{const t=await N(s,n);alert("Bid placed successfully!"),location.reload()}catch(t){console.error(t),alert("Failed to place bid. Please try again.")}})}catch(e){console.error("Error displaying single listing:",e)}}U();
