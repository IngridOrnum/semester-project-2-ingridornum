import{A as l,c as g}from"./constants-BDf8nfUD.js";import{g as u,r as m}from"./main-Cx0DgoeT.js";async function I(e=40,t=1,o="latest",a="",n="all"){const i=new URLSearchParams({limit:e,page:t,_seller:!0,_bids:!0});o==="latest"?(i.append("sort","created"),i.append("sortOrder","desc")):o==="a-z"&&(i.append("sort","title"),i.append("sortOrder","asc")),n==="active"&&i.append("_active","true"),a&&i.append("q",a);const d={method:"GET"},s=await fetch(`${l}?${i.toString()}`,d);if(console.log(`${l}?${i.toString()}`),!s.ok)throw new Error(`Error fetching data: ${s.status} ${s.statusText}`);const c=await s.json();return{listings:c.data||[],meta:c.meta||{}}}async function h(e){const t=await u(),o={method:"GET",headers:{"X-Noroff-API-Key":g,Authorization:`Bearer ${t}`,"Content-Type":"application/json"}},a=await fetch(`${l}/${e}?_bids=true&_seller=true`,o);if(!a.ok)throw console.error("Failed to fetch profile data:",a),new Error(`Error fetching profile data: ${a.status}`);const n=await a.json();return console.log(n),n}async function b(e,t){const o=await u(),a={method:"POST",headers:{"X-Noroff-API-Key":g,Authorization:`Bearer ${o}`,"Content-Type":"application/json"},body:JSON.stringify({amount:t})};console.log("Sending API request to:",`${l}/${e}/bids`,a);const n=await fetch(`${l}/${e}/bids`,a);if(!n.ok){const d=await n.text();throw console.error("Failed to place bid:",n.status,n.statusText,d),new Error(`Error placing bid: ${n.status}`)}return await n.json()}async function $(e){const t=new Date(e),o=t.toLocaleDateString("en-GB",{day:"2-digit",month:"2-digit",year:"2-digit"}),a=t.toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit",hour12:!0});return`${o} ${a}`}async function w(){const e=localStorage.getItem("loggedInUsername");if(!e)throw new Error("No logged-in user found");return(await m(e)).data.credits}async function v(e){return!e||!e.length?(console.log("No bids found."),"No bids"):Math.max(...e.map(t=>t.amount))}async function A(){const e=localStorage.getItem("listingId");if(!e){console.error("no listing id found");return}try{const t=await h(e),o=await v(t.data.bids),a=await w(),n=document.getElementById("single-listing-container"),i=new Date,d=new Date(t.data.endsAt),s=i>d,c=await $(t.data.endsAt);n.innerHTML=`
