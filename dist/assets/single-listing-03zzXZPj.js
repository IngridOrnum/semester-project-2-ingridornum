import{A as d,c as g}from"./constants-BDf8nfUD.js";import{g as u,r as m}from"./main-Cx0DgoeT.js";async function I(e=40,t=1,o="latest",n=""){const a=new URLSearchParams({limit:e,page:t,_seller:!0,_bids:!0});o==="latest"?(a.append("sort","created"),a.append("sortOrder","desc")):o==="a-z"&&(a.append("sort","title"),a.append("sortOrder","asc")),n&&a.append("q",n);const c={method:"GET"},i=await fetch(`${d}?${a.toString()}`,c);if(console.log(`${d}?${a.toString()}`),!i.ok)throw new Error(`Error fetching data: ${i.status} ${i.statusText}`);const s=await i.json();return{listings:s.data||[],meta:s.meta||{}}}async function h(e){const t=await u(),o={method:"GET",headers:{"X-Noroff-API-Key":g,Authorization:`Bearer ${t}`,"Content-Type":"application/json"}},n=await fetch(`${d}/${e}?_bids=true&_seller=true`,o);if(!n.ok)throw console.error("Failed to fetch profile data:",n),new Error(`Error fetching profile data: ${n.status}`);const a=await n.json();return console.log(a),a}async function b(e,t){const o=await u(),n={method:"POST",headers:{"X-Noroff-API-Key":g,Authorization:`Bearer ${o}`,"Content-Type":"application/json"},body:JSON.stringify({amount:t})};console.log("Sending API request to:",`${d}/${e}/bids`,n);const a=await fetch(`${d}/${e}/bids`,n);if(!a.ok){const i=await a.text();throw console.error("Failed to place bid:",a.status,a.statusText,i),new Error(`Error placing bid: ${a.status}`)}return await a.json()}async function $(e){const t=new Date(e),o=t.toLocaleDateString("en-GB",{day:"2-digit",month:"2-digit",year:"2-digit"}),n=t.toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit",hour12:!0});return`${o} ${n}`}async function w(){const e=localStorage.getItem("loggedInUsername");if(!e)throw new Error("No logged-in user found");return(await m(e)).data.credits}async function y(e){return!e||!e.length?(console.log("No bids found."),"No bids"):Math.max(...e.map(t=>t.amount))}async function A(){const e=localStorage.getItem("listingId");if(!e){console.error("no listing id found");return}try{const t=await h(e),o=await y(t.data.bids),n=await w(),a=document.getElementById("single-listing-container"),c=new Date,i=new Date(t.data.endsAt),s=c>i,p=await $(t.data.endsAt);a.innerHTML=`
